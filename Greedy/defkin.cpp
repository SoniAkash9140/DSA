...
int solve(int row, int col, vector<pair<int,int>> &cor, int n)
{
    vector<int> rowLoc;
    rowLoc.push_back(0);
    ...
    for(int i=0;i<n;i++)
        rowLoc.push_back(cor[i].first);

    sort(rowLoc.begin(), rowLoc.end());

    int maxRowDiff = 0;

    for(int i=1;i<rowLoc.size();i++)
    {
    ...
    for(int i=0;i<n;i++)
        colLoc.push_back(cor[i].second);

    sort(colLoc.begin(), colLoc.end());

    int maxColDiff = 0;

    for(int i=1;i<colLoc.size();i++)
    {
